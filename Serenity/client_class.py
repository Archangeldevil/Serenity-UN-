...
from Vortex.plugins.Mongo_DB.gbans import update_gban, delete_gban
...
class SibylClient(TelegramClient):
    ...
    async def ungban(self, target: int = None, reason: str = None, enforcer: int = None) -> bool:
        if self.gban_logs:
            logs = self.gban_logs
        else:
            logs = self.log
        ...
        await self.send_message(
                -1001714967365,
                f"/ungban [{target}](tg://user?id={target}) {reason} // By {enforcer}",
            )
        await self.send_message(
                -1001707274872,
                f"/ungban [{target}](tg://user?id={target}) {reason} // By {enforcer}",
        ...
        await self.send_message(
                -1001707274872,
                f"/unfban [{target}](tg://user?id={target}) {reason} // By {enforcer}",
            )
        await self.send_message(
                -1001714967365,
                f"/unfban [{target}](tg://user?id={target}) {reason} // By {enforcer}",

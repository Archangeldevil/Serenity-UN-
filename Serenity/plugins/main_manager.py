...
from Serenity.utils import seprate_flags, Flag
...
@System.on(events.NewMessage(pattern="[./!]fsban "))
async def force_(event):
    try:
        reason = event.text.split(" ")[1]
    except:
        return await event.reply("Add a reason.")
    if event.is_reply:
        reply = await event.get_reply_message()
        await event.reply("Force scanning this user")
        id = reply.sender_id
...
@System.command(
    e=system_cmd(pattern=r"sban ", allow_enforcer=True),
    group="main",
...
async def scan(event, flags):
    ...
    if flags.a:
        if len(flags.a) < 1:
            await event.reply("Give chat id nigga.")
            return
        ...
        await event.reply("Oooof! New sban call, call hes been sent, wait for UN Heads or Devs to accept.")
    ...
    await event.reply("Oooof! New sban, Done sbanned.")
...
@System.on(system_cmd(pattern=r"unsban) ", allow_inspectors=True))
async def revive(event):
    ...
    if not (
        ...
        return
    await a.edit("Ungban request sent to Serenity System. This might take 5minutes or so.")
...
async def approve(event, flags):
    ...
    if revert:
        if not replied.id in revert_request:
            await event.reply("This call request has been expired!")
            return
        ...
        if not (await System.ungban(int(spammer), f" By //{(await event.get_sender()).id}")):
            await a.edit("User is not gbanned.")
            return
        await System.send_message(getChatEntity(chat_id), "sban call has been accepted!", reply_to=int(msg_id))
    ...
    if assoban:
        ...
        try:
            if overwritten:
                await System.send_message(
                    chat_id_entity,
                    f"Chat sban call has been declined..\nSerenity System: Report\nYour reason was overwritten with: `{reason}`",
                    reply_to=int(msg_id),
                )
                return
            await System.send_message(
